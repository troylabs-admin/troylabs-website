...
function renderTable() {
  ...
  tbody.innerHTML = members.map((r) => {
    ...
    return `<tr data-id="${r.id}" data-status="${status}" data-cohort="${cohortOf(r.join_term, r.join_year)}" data-division="${esc((r.divisions?.[0] ?? '').replace('PRODUCT MANAGEMENT', 'PRODUCT'))}" data-text="${esc(`${r.full_name} ${r.usc_email ?? ''} ${r.personal_email ?? ''} ${r.current_company ?? ''} ${city} ${division}`.toLowerCase())}">
      <td>${esc(r.full_name || '(no name yet)')}${admins.has(r.id) ? ' <span class="t-fine portal-role">ADMIN</span>' : ''}</td><td class="text-muted">${esc(email)}</td><td><span class="t-fine portal-tag">${status}</span></td><td class="text-muted">${cohortOf(r.join_term, r.join_year) || '—'}</td><td class="text-muted">${esc(division) || '—'}</td><td class="text-muted">${esc(city) || '—'}</td><td>${completeness(r)}%</td>
      <td style="text-align:right;white-space:nowrap"><button type="button" class="t-label portal-linklike" data-roles-for="${r.id}" style="margin-right:calc(16 * var(--u))">ROLES</button><button type="button" class="t-label portal-linklike" data-admin-toggle="${r.id}" style="margin-right:calc(16 * var(--u))"${r.id === myId ? ' disabled title="You cannot change your own admin access"' : ''}>${admins.has(r.id) ? 'REMOVE ADMIN' : 'MAKE ADMIN'}</button><button type="button" class="t-label portal-linklike" data-remove="${r.id}">REMOVE ACCESS</button></td>
    </tr>`;
